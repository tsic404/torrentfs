...
//! single `length`/`name` pair. Public sample torrents usually have no
//! reachable seeders, so multi-file read scenarios need a deterministic local
//! swarm too. Flow: (1) start a minimal HTTP tracker, (2) stream every file
//! under the payload directory into the seed tree hashing the concatenated
//! byte stream piece-by-piece (pieces span file boundaries), (3) bencode a
//! multi-file `.torrent`, (4) run a libtorrent session in seeding state until
//! killed. The tracker, bencoding helpers, and keep-alive loop are shared with
//! `torrentfs-selfseed-env` via [`seeder_common`] (`ci/seeder_common.rs`).
